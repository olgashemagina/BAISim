...
В папке `examples/` находятся несколько `.bat` файлов, показывающих процесс поэтапного обучения каскадного детектора лица. Например, `build_face_ms1000_ff0p1.bat` загружает конфигурацию детектора, добавляет новый сильный классификатор и запускает дообучение каскада на обновленных данных. Для корректной работы необходимы установленные базы Face_Test и Background.
